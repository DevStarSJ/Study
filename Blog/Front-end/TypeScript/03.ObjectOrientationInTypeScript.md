...
`fs.readdirSync`와 `fs.statSync` 메서드는 **NodeJS**에 있는 것으로 입니다.
...
`HeatSensor의 생성자로 `ControlPanel` 객체를 전달합니다.
...
#### 그림 3-3 추상 팩토리 패턴
![그림 3-3 추상 팩토리 패턴](./image/TypeScript.03.03.png)
...
이 함수는 각각의 *인자화된 클래스*(argumented class) 배열을 baseCtors로 전달하고 *구현할 클래스*를 derivedCtor로 전달합니다. 
...
`Listing 3-20`에는 재사용 가능한 *인자화된 클래스*가 정의 되어 있습니다.
...
타입스크립트에서는 `implements` 키워드 뒤에 믹스인 리스트(*인자화된 클래스*)를 콤마로 나열해서 클래스를 구성할 수 있습니다.
...
*인자화된 클래스*에서 제공받을 수 있는 공간만을 할당해놓고 있습니다.
...
왜냐면 컴파일러가 *인자화된 클래스*가 구현이 안되었을 경우 오류를 발생합니다.
또한 *믹스인 함수*와 *인자회된 클래스*가 둘 다 동일한 이름의 `private` 멤버를 가지고 있는 경우에도 오류를 발생합니다.

**믹스인**의 또다른 제한점은 함수는 *인자화된 클래스*로 매핑되지만, 속성은 안됩니다. (`List 3-23`)
*인자화된 클래스* 안에 속성을 구현한 경우 *믹스인 함수*에서 초기화해줘야 합니다.
실수를 방지하기 위해서는 *인자화된 클래스* 안에 있는 속성에 대해서는 디폴트값을 정의하지 않는 것이 좋습니다.
