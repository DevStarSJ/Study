...
다만 그 판단을 OS에서 해서 실행시키는 것이니깐, C++ 개발자 입장에서는 내가 직접 다 관리하는게 아니면 믿지 못한다고 생각하는 경우가 많으니깐요.  

그래서 아래와 같이 `std::async`대신 `std::packaged_task`를 활용해서 일단 thread는 내가 직접 해제하는 예제가 있습니다.  
예제가 너무 일반적인 경우를 들어서 보기에 좀 복잡합니다.
...
책의 예제는 일부러 좀 복잡하게 보이게 하고자한 저자의 의도가... ;;; 이분 참... 왜이럴까요 ? 너무 일반적으로 설명을 ...  
물론 일반적인 예제도 좋지만, 덜 일반적이라도 독자들이 그걸 보고 상황에 맞게 고쳐서 쓸수 있을텐데 말이죠.  
전 그냥 덜 일반적으로 예제를 구현하겠습니다.

#####Listing 4.14 : A sample implementation of spawn_task (덜 일반적인 예제)
```C++
template<typename T>
std::future<std::list<T>> spawn_task(std::future<std::list<T>>&& f, std::list<T>>&& a)
{
     // packaged_task를 이용한 작업 생성.
     std::packaged_task<std::list<T>(std::list<T>)> task( parallel_quick_sort );
...
multi-thread 프로그래밍을 할려면 신경써야 할 것이 참 많습니다.  
