...
![image](https://github.com/DevStarSJ/Study/blob/master/Blog/Oracle/sqlp/image/03.07.sql.adv.01.png?raw=true)
...
SELECT a.고객번호, a.연체금액, a.연체기간
  FROM 고객별연체금액 a
 WHERE a.고객번호 = :no
   AND a.rowid = (SELECT /*+ index(b IDX_고객별연체금액_고객_일자 desc) */ rowid
                       FROM 고객별연체금액 b
                      WHERE a.고객번호 = b.고객번호
                        AND b.시작일자 <= :date
                        AND rownum <= 1);
