...
SELECT DEPTNO, ENAME, SAL,
       (SELECT AVG(SAL) FROM EMP B WHERE B.DEPTNO = A.DEPTNO GROUP BY B.DEPTNO) AS AVG_SAL
  FROM EMP A;
```
...
SELECT A.DEPTNO, A.ENAME, A.SAL, B.AVG_SAL
  FROM EMP A,
       (SELECT DEPTNO, AVG(SAL) AS AVG_SAL FROM EMP GROUP BY DEPTNO) B
...
MANAGER 중 연봉 BEST 5를 순서대로 출력하고자 할 경우
```SQL
SELECT *
  FROM (SELECT * FROM EMP WHERE JOB = 'MANAGER' ORDER BY SAL DESC)
 WHERE ROWNUM <= 5;
```
